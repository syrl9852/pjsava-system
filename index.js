...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "register")
    return sqlconnect(interaction, connection, 1);
  // 削除
  if (interaction.commandName === "reset")
    return sqlconnect(interaction, connection, 2);
  // 更新
  if (interaction.commandName === "check-in")
    return sqlconnect(interaction, connection, 3);
...
function sqlconnect(interaction, connection, mode) {
  ...
  connection.query(
    `select * from oshirole where user = ${userId}`,
    function (error, results, fields) {
      ...
      if (result) {
        switch (mode) {
          ...
          case 1:
            ...
            if (!restar) {
              // 初期設定済み
              connection.query(
                `update oshirole set ${target}=1, lastdate=${miDate} where user = ${userId}`
              );
              interaction.reply({
                content: "登録が完了しました！\n更新は/check-inから出来ます。",
                ephemeral: true,
          ...
          case 3:
            if (miDate > result.lastdate + 2592000000) {
              ...
              const nextCan = moment(new Date(result.lastdate + 2592000000))
                .tz("Asia/Tokyo")
        ...
        if (mode == 1) {
          connection.query(
            `insert into oshirole (user, lastdate, ${target}) values (${userId}, ${miDate}, 1)`
          );
          interaction.reply({
            content: "登録が完了しました！\n更新は/check-inから出来ます。",
            ephemeral: true,
...
async function memberCount() {
  ...
  const user =
    all - guild.members.cache.filter((member) => member.user.bot).size;
